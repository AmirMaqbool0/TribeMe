  ...
  async ({ file, brandId }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.put<UploadImageResponse>(
        `${process.env.NEXT_PUBLIC_BASE_URI}/brand/upload-offer-image/${brandId}`,
        formData,
        {
          headers: {
...
export const selectUploadImageState = (state: { uploadImage: UploadImageState }) =>
  state.uploadImage;
export const selectUploadImageStatus = (state: { uploadImage: UploadImageState }) =>
  state.uploadImage.status;
export const selectUploadImageError = (state: { uploadImage: UploadImageState }) =>
  state.uploadImage.error;
export const selectUploadImageUrl = (state: { uploadImage: UploadImageState }) =>
